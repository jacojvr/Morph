...
def deformTETsurf(NC,inner,outer,neighbListTet,NCsurF,incr=10,smoothIncr=20):
  for i in range(incr):
...
def elasticsurf(NCB_all,NCB_triList,ConnectTetB,ConnectB,LandmB,LandmB_NC,AllowableBI,NCT,ConnectT,AllowableT,UseN_B,UseN_T,k_max,USENORMALS):
...
def MeshTaubin(nodes,nodeList,NC,neighbList,Umb,listPos):
  for i in nodes:
    nd = nodeList[i]
    if listPos==0:
      adj=neighbList[i]
      ...
      adj=neighbList[nd]
    NC[nd,:] = NC[nd,:] + Umb[nd,:]*0.235 -0.265*(np.sum(Umb[adj,],0))/(adj.size)
