...
def deformTETsurf(NC,outer,neighbListTet,NCsurF,incr=10,smoothIncr=20):
  for i in range(incr):
    fact = (i+1)*(1./smoothIncr)
    NCP = np.r_[NC]
    NCN = RBFmorph(NC,NC[outer,],(NCsurF-NC[outer,])/fact)
    Disp = NCN-NCP
    Disp2 = MeshSmooth(np.array(range(NC.shape[0])),Disp,neighbListTet,smoothIncr)
    NC = NC+Disp2
  return NC

def elasticsurf(NCBtet,ConnectTetB,NCB,ConnectB,LandmB,LandmB_NC,AllowableBI,NCT,ConnectT,AllowableT,UseN_B,UseN_T,k_max=100,USENORMALS=np.array([]),SmIt = 10,DumpSm = 5):
  ...
  NCB_triList = np.ma.load('BSkullFTn_outer')
  NCB_internal = np.ma.load('BSkullFTn_inner')
  #print 'Get list of Base nodes to be used as internal nodes'
  #NCB_internal = np.array(range(NCB_all.shape[0]))
  #for i in NCB_triList:
    #NCB_internal = NCB_internal[NCB_internal<>i]
  
  # determine N1,N2,T1 and T2:
  N1 = NCB_triList.size
  N1T = NCBtet.shape[0]
  N1Tint = NCB_internal.size
  N2 = NCT.shape[0]
  T1=ConnectB.shape[0]
  T2=ConnectT.shape[0]
  NL=LandmB.shape[0]
  ...
  #calc = results.manage(pprocess.MakeParallel(Get1neigh))
  #for j in range(0,LIM):
      #calc(NCB_internal[np.array(range(0,NPP1T))+j*NPP1T],NCB_all,ConnectTetB)
  #for j in range(0,LIM):
      #neighbListTet[j*NPP1:(1+j)*NPP1] = results[j]
  #neighbListTet[NCB_internal[LIM*NPP1:N1]]=Get1neigh(NCB_internal[np.array(range(LIM*NPP1,N1))],NCB_all,ConnectTetB)
  ...
  #W_km1A = np.r_[NCBtet]
  #W_km1p = np.r_[NCBtet]
  #while (k<k_max)|(Err>Tol):
    #k=k+1
    #print 'Iteration : ',k
    #DS = np.zeros((N1T,3))
    #knownC = W_km1A[NCB_triList[LandmB],]
    #knownD = LandmB_NC-knownC
    ...
    #for j in range(0,LIM):
      #calc(np.array(range(0,NPP1T))+j*NPP1T,W_km1A,knownC,knownD,sigma_k2,gamm)
    #for j in range(0,LIM):
      #DS[np.array(range(0,NPP1T))+j*NPP1T,:] = results[j]
    #DS[range(LIM*NPP1T,N1T),:]=GaussianSmooth(np.array(range(LIM*NPP1T,N1T)),W_km1A,knownC,knownD,sigma_k2,gamm)
    #DS[np.isnan(DS)]=0
    #W_km1A = W_km1A+DS
    #Err = np.sum(np.sqrt(np.sum(DS*DS,1)),0)/N1T
  #Disp = W_km1A - W_km1p
  #print 'Determining smooth displacement using element connectivity'
  #DispS = MeshSmooth(np.array(range(N1T)),Disp,neighbListTet,SmIt)
  W_km1A = np.ma.load('TempElasNodes_Iter0_TimeWedOct20')#W_km1p+DispS
  W_km1p = np.r_[W_km1A]
  np.ma.dump(W_km1A,'TempElasNodes_Iter0_Time'+time.ctime())
  ...
  while k<=k_max:
    ...
    calc = results.manage(pprocess.MakeParallel(GaussianSmooth))
    for j in range(0,LIM):
      calc(np.array(range(0,NPP1T))+j*NPP1T,W_km1A,NCp,DD,sigma_k2,gamm)
    for j in range(0,LIM):
      DS[np.array(range(0,NPP1T))+j*NPP1T,:] = results[j]
    DS[range(LIM*NPP1T,N1T),:]=GaussianSmooth(np.array(range(LIM*NPP1T,N1T)),W_km1A,NCp,DD,sigma_k2,gamm)
    print "			",time.clock()-tic," seconds"
    ...
    TotalMorph = np.sum(np.sqrt(np.sum(DS*DS,1)),0)/NCBtet.shape[0]
    ...
    if (k>2)&(np.mod(k-1,DumpSm)==0):
      ...
      W_km1A = W_km1p+DispS
      W_km1p = np.r_[W_km1A]
      np.ma.dump(W_km1A,'TempElasNodes_Iter'+str(k-1)+'_Time'+time.ctime())
      print "		Smoothing done in ",time.clock()-tic," seconds"
    #print "COARSE SURFACE REGISTRATION"
    #print "	using landmark displacements to deform using RBF"
...
def MeshSmooth(nodes,NC,neighbList,Iter,listPos=0):
  ...
  for i in range(Iter):
    ...
    for j in range(0,LIM):
	calc(np.array(range(0,NPP1))+j*NPP1,nodes,NC,neighbList,Umb,listPos)
    for j in range(0,LIM):
	Umb[nodes[np.array(range(0,NPP1))+j*NPP1],:] = results[j]
    Umb[nodes[range(LIM*NPP1,N1)],:]=MeshUmbrella(range(LIM*NPP1,N1),nodes,NC,neighbList,Umb,listPos)
    ...
    NC[nodes[range(LIM*NPP1,N1)],:]=MeshTaubin(range(LIM*NPP1,N1),nodes,NC,neighbList,Umb,listPos)
  return NC
...
def MeshUmbrella(nodes,nodeList,NC,neighbList,Umb,listPos):
  for i in nodes:
    nd = nodeList[i]
    if listPos==0:
      adj=neighbList[i]
      ...
      adj=neighbList[nd]
    Umb[nd,:] = np.sum(NC[adj,],0)/(adj.size) - NC[nd,]
  return Umb[nodeList[nodes],:]

def MeshTaubin(nodes,nodeList,NC,neighbList,Umb,listPos,aa=0.235,bb=0.265):
  for i in nodes:
    nd = nodeList[i]
    if listPos==0:
      adj=neighbList[i]
      ...
      adj=neighbList[nd]
    NC[nd,:] = NC[nd,:] + Umb[nd,:]*aa -bb*(np.sum(Umb[adj,],0))/(adj.size)
  return NC[nodeList[nodes],:]
...
def RBFmorph(NC,knownC,knownD):
  ...
  for i in range(0,nrB):
    TEMP = np.array([np.sqrt(np.sum(np.power((np.ones((nrB,1))*knownC[i,:]-knownC),2),1))])*10
    ##### Thin plate spline:
    TEMP = TEMP*TEMP*np.log10(TEMP)
    ##### Gauss weight:
    #TEMP = np.exp(-TEMP*TEMP/10)
  ...
  for i in range(0,nrN):
    TEMP = np.sqrt(np.sum(np.power((np.ones((nrB,1))*NCB[i,:]-knownC),2),1))*10
    ##### Thin Plate Spline:
